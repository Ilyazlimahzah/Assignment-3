...
const requestHandler = async function (req, res) {
	...
	if (req.url === '/books' && req.method === 'GET') {
	...
	} else if (req.url === '/author' && req.method === 'POST') {
		//authentication
        authenticate(req, res)
        .then(() => {
            addAuthor(req, res);
		...
		updateBook(req, res);
	} else if (req.url === '/author' && req.method === 'PUT') {
		//authentication
		res.writeHead(201);
        res.end(
            JSON.stringify({
		...
		deleteBook(req, res);
	} else if (req.url === '/author' && req.method === 'DELETE') {
		//authentication
		res.writeHead(204);
        res.end(
            JSON.stringify({
...
//RETREIVE ALL Authors ==> GET /author
...
const addAuthor = function (req, res) {
	...
	req.on('end', () => {
		const parsedBody = Buffer.concat(body).toString(); // concatenate raw data into a single buffer string
		const newAuthor = JSON.parse(parsedBody); // parse the buffer string into a JSON object

		// get ID of last book in the database
		const lastAuthor = authorsDB[authorsDB.length - 1];
		const lastAuthorId = lastAuthor.id;
		newAuthor.id = lastAuthor + 1;

		//save to db
		authorsDB.push(newAuthor);
		fs.writeFile(authorsDbPath, JSON.stringify(authorsDB), (err) => {
			if (err) {
				console.log(err);
				res.writeHead(500);
				res.end(
